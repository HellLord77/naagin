...
@router.get("")
async def get(session: SessionDependency, owner_id: OwnerIdDependency) -> FurnitureMySetGetResponseModel:
    furniture_myset_list = await session.find_all(FurnitureMySetSchema, FurnitureMySetSchema.owner_id == owner_id)

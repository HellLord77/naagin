...
async def provide_girl_statuses(master_version: MasterVersionHeader) -> list[GirlStatusCSVModel]:
    ...
    return [
        GirlStatusCSVModel(appeal=appeal, power_or_technic=power_or_technic, stamina=stamina)
        for appeal, power_or_technic, stamina in islice(zip(appeals, powers_or_technics, staminas, strict=True), count)
    ]
